...
void filament_view_set_visible_layers(View* view, uint8_t select, uint8_t values) {
    if (!view) return;
...
void filament_renderable_builder_layer_mask(RenderableBuilderWrapper* wrapper, uint8_t select, uint8_t values) {
    if (!wrapper || !wrapper->builder) return;
    wrapper->builder->layerMask(select, values);
}
...
void filagui_imgui_helper_render_scene_ui(
    ...
    helper->render(delta_seconds, [=](filament::Engine*, filament::View*) {
        ...
        // Transform gizmo is now rendered/picked in Filament editor overlay pass.
...
void filament_renderable_set_layer_mask(
    Engine* engine,
    int32_t entity_id,
    uint8_t select,
    uint8_t values
    ...
    auto instance = rm.getInstance(entity);
    if (!instance) return;
    rm.setLayerMask(instance, select, values);
}
